...
const TutorBankBox = ({ bankName, bankNumber }: { bankName: string, bankNumber: string }) => {
  const banks = [
    "Bangkok Bank",
    "Kasikorn Bank",
    ...
    <Card className='w-full'>
      <CardHeader className="flex items-center justify-between">
        <h2 className="text-lg font-semibold flex items-center">
          <CreditCard className="w-5 h-5 mr-2 text-muted-foreground" /> Bank Account
        </h2>
        <Button variant="ghost" size="icon" onClick={handleToggle} className='hover:bg-orange-200 cursor-pointer'>
      ...
      <CardContent className="space-y-4 w-full">
            ...
            <DropdownMenuContent className="xl:w-[50rem] lg:w-[40rem] md:w-[25rem] w-[20rem]">
              ...
              >
                {banks.map((bank, index) => (
                  <DropdownMenuRadioItem key={index} value={bank} className="w-full">
                    {bank}
        ...
        <div className="w-full">
          ...
          <Input placeholder="Enter account number" defaultValue={bankNumber} disabled={disable} className="w-full" />
        </div>
      </CardContent>
    </Card>
  )
