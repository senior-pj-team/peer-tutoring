...
const RefundStatus = async ({ sessionId }: { sessionId: number | null }) => {
  ...
  if (!data || data.length<1) return <div className="text-sm text-gray-500">No refund request found.</div>;
  ...
  const createdDate = format(parseISO(refund_report.created_at),  "yyyy MMMM dd");
